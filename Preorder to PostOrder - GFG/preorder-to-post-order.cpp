...
public:
    ...
    Node* construct(int pre[],int &idx,int key,int mn,int mx,int n){
        if(idx>=n){
            return NULL;
        }
        Node *root=NULL;
        if(key>mn&& key<mx){
            ...
            idx=idx+1;
            if(idx<n){
                root->left=construct(pre,idx,pre[idx],mn,key,n);
            }
            if(idx<n){
                root->right=construct(pre,idx,pre[idx],key,mx,n);
            }
    ...
    {
        int idx=0;
        return construct(pre,idx,pre[0],INT_MIN,INT_MAX,size);
    }
};
